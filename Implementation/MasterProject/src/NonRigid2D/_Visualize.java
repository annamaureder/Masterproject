...
public class _Visualize {
	...
	public static void colorClusters(List<_Cluster[]> segments, String title) {

		ColorProcessor segmentation = new ColorProcessor(_Segmentation.width, _Segmentation.height);
		ColorProcessor segmentation2 = new ColorProcessor(_Segmentation.width, _Segmentation.height);
		segmentation.invert();
		segmentation2.invert();

		for (int i = 0; i < segments.size(); i++) {

			_Cluster[] clusters = segments.get(i);

			_Cluster cluster1 = clusters[0];
			_Cluster cluster2 = clusters[1];
	...
	public static void drawSkeleton(ColorProcessor cp, ColorProcessor cp2, List<_Cluster[]> segments){

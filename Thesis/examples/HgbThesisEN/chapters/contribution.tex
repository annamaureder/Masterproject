...
\begin{algorithm}[tbp]
	...
	\begin{algorithmic}[1]     % [1] = all lines are numbered
		\label{inputClusters}
	\Procedure{Subclustering}{$clusterA, clusterB$}
		
		\State $subclusters \gets ()$
		\State $left \gets nil$
		\State $right \gets nil$
		\State $node:=<clusterA, clusterB, left, right>$
		\State $subclusters$ = \Call{Subdivide}{$node$, $subclusters$}
...
\begin{algorithm}[tbp]
	...
	\begin{algorithmic}[1]     % [1] = all lines are numbered
		\label{clustering}
		\Procedure{Subdivide}{$node$, $subclusters$} 
		
		\Procedure{Divide}{$node$}
		
		\If {\Call{match}{$clusterA(node)$, $clusterB(node)$}}
		...
		\State \Call{Divide}{$left(node)$}
		\State \Call{Divide}{$right(node)$}

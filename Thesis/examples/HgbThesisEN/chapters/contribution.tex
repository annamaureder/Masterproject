...
%%
\begin{itemize}
	\item By recursively subdividing $C_1$ and $C_2$ a sub cluster might actually consist of more than one point clusters located apart from each other. 
...
\end{itemize}
%%
As a result, the approach needs to be extended. Additionally, for simple objects the detection of the joints is not exact although the segmentation in the approximate rigid parts succeeds. Also for that improvements have to be done, which for example include the special treatment for points located near to a joint.
